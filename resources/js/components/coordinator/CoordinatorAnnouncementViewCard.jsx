  ...
  return (
    <div className="w-full space-y-6">
        ...
        <div className="flex flex-wrap justify-center lg:justify-start gap-3 w-full">
          {image.map((img, index) => (
            <img
              key={index}
              src={img}
              className="w-full sm:w-[45%] lg:w-[250px] max-h-60 object-contain rounded-md shadow"
            />
          ))}
        </div>
      ) : typeof image === "string" ? (
      ...
      {isRevise && revision_note && (
        <div className="mt-4 rounded-xl border-l-4 border-yellow-400 bg-white p-4 shadow-sm w-full">
