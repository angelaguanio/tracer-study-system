...
export default function RecentActivityList({ title, items, linkPattern, emptyMessage = 'No recent activity' }) {
  ...
  return (
    <Card className="hover:shadow-md transition-shadow duration-300 w-full gap-3">
              ...
              >
                <div className="flex justify-between items-start gap-2">
                  ...
                  <div className="flex flex-col items-end gap-1 shrink-0">
                    <div className="text-[11px] md:text-xs text-muted-foreground whitespace-nowrap">
                      {item.created_at || item.submitted_at}
                    </div>
                    <ArrowRight className="h-3 w-3 md:h-4 md:w-4 text-muted-foreground group-hover:text-primary group-hover:translate-x-1 transition-all" />
                  </div>
                </div>
                {item.status && (
