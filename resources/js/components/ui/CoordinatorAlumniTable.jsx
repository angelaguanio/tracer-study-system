    ...
    <div className="flex flex-col gap-3">
          ...
          <Table className="w-full table-fixed min-w-[700px]">
            ...
            <TableBody>
                ...
                >
                  ...
                  <TableCell className="pl-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-medium text-gray-800 truncate">{item.name ?? "Unknown"}</span>
                    </div>
                  </TableCell>
                  <TableCell className="text-center">
                  ...
                  <TableCell className="text-center text-gray-600 font-medium">{item.year ?? "N/A"}</TableCell>
                  <TableCell className="text-center">
              ...
              {alumniData.length < rowsPerPage &&
                Array.from({ length: rowsPerPage - alumniData.length }).map((_, i) => (
      ...
      <div className="flex justify-end mt-2">
        <div className="flex items-center gap-1">
          <button onClick={() => currentPage > 1 && goToPage(currentPage - 1)} disabled={currentPage === 1} className="w-9 h-9 flex items-center justify-center rounded-lg border bg-white shadow-sm hover:bg-gray-50 disabled:opacity-40"><ChevronLeft size={16} /></button>
          {getPaginationItems().map((item, i) => (
            <button key={i} onClick={() => typeof item === 'number' && goToPage(item)} className={`w-9 h-9 rounded-lg text-sm border font-medium ${currentPage === item ? "bg-blue-500 text-white" : "bg-white text-gray-600"}`}>{item}</button>
          ))}
          <button onClick={() => currentPage < lastPage && goToPage(currentPage + 1)} disabled={currentPage === lastPage} className="w-9 h-9 flex items-center justify-center rounded-lg border bg-white shadow-sm hover:bg-gray-50 disabled:opacity-40"><ChevronRight size={16} /></button>
        </div>
      </div>
    </div>
  );
