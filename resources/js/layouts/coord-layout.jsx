...
export default function CoordinatorSurveyResponse({ responses, filters, survey }) {
  ...
  // Sync local state when parent route updates (page/filter changes)
  useEffect(() => {
  ...
  const confirmDelete = () => {
    ...
    // Delete route endpoint structure: DELETE /survey-response/{surveyId}/{userId}
  ...
  return (
    <div className="w-full max-h-full flex flex-col overflow-hidden p-4">
      {/* Title block and Filters grouped in a single main flex element */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6 w-full shrink-0">
      ...
      {/* Table block layout container with custom fixed-header rendering architecture */}
