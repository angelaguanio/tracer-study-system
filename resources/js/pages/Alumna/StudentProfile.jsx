...
export default function StudentProfile() {
    ...
    const avatarElement = profile?.profile_picture ? (
        ...
        <div className="h-full w-full bg-gradient-to-br from-gray-600 to-gray-800 text-white flex items-center justify-center text-xl font-bold">
            {(profile?.first_name?.[0] || 'R') + (profile?.last_name?.[0] || 'R')}
        </div>
    );

    // Helper to check present status
    const isPresent = (val) => val == 1 || val === true;
                        ...
                        <div className="grid grid-cols-2 gap-y-6">
                            ...
                            <div className="flex flex-col text-left">
                                ...
                                <p className="text-[13px] font-bold text-[#343a40]">
                                    {emp.employment_start_year || '—'} - {isPresent(emp.is_present) ? 'Present' : (emp.employment_end_year || '—')}
                                </p>
                            </div>
                            <div>
                                    ...
                                    .map((history) => (
                                        <tr key={history.id} className="hover:bg-gray-50 transition-colors">
                                            <td className="py-3 pl-4 text-gray-600 font-medium truncate">
                                                {history.employment_start_year || '—'} - {isPresent(history.is_present) ? 'Present' : (history.employment_end_year || '—')}
                                            </td>
                                            ...
                                            <td className="py-3 text-center pr-4">
                                                <Link href={route('alumna.history.show', { id: history.id })} className="inline-block text-[10px] font-bold text-blue-500 border border-blue-400 hover:bg-blue-50 px-3 py-1.5 rounded transition-colors">
                                                    View Details
                                                </Link>
                                            </td>
                                        </tr>
