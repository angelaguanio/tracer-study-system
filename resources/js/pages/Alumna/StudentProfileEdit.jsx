...
export default function StudentProfileEdit() {
    ...
    const handleSubmit = (e) => {
        ...
        const payload = {
            ...
            employment_end_year: data.is_employed === 'yes' && data.employment_end_year === 'current' ? null : data.employment_end_year,
        };
        post(route('alumna.profile.update'), { data: payload, forceFormData: true, preserveScroll: true });
    };
    ...
    return (
        <div className="w-full max-w-[800px] mx-auto py-8 px-4 flex flex-col gap-5 pb-12">
            <div className="flex justify-between items-center">
                ...
                <button form="profile-form" type="submit" disabled={processing} className="flex items-center bg-[#008542] hover:bg-green-800 text-white text-[11px] font-bold px-4 py-2 rounded shadow-sm uppercase tracking-wide transition-all">
            ...
            <form id="profile-form" className="flex flex-col gap-6" onSubmit={handleSubmit}>
                    ...
                    <div className="flex items-center gap-5 mb-4">
                        <div className="relative group">
                            <div className="h-16 w-16 rounded-full overflow-hidden shadow-inner border-2 border-gray-200 group-hover:border-[#008542] transition-all">
                                {avatarPreview ? <img src={avatarPreview} alt="Profile" className="w-full h-full object-cover" /> : <div className="h-full w-full bg-[#6c757d] text-white flex items-center justify-center text-xl font-bold">{profile?.initials || 'U'}</div>}
                            </div>
                            <input ref={fileInputRef} type="file" accept="image/*" onChange={handleFileChange} className="hidden" />
                            <button type="button" onClick={() => fileInputRef.current?.click()} className="absolute -bottom-1 -right-1 bg-white p-1 rounded-full shadow-lg border hover:shadow-xl transition-all"><IconImage /></button>
                        </div>
                    </div>
                ...
                {/* EMPLOYMENT STATUS */}
                <section className="bg-white rounded-xl shadow-sm border border-gray-50 p-6 sm:p-8 flex flex-col gap-6">
                    <div className="flex items-center gap-2 text-gray-600 font-bold text-[13px] uppercase tracking-tight"><IconBriefcase /> Employment Status</div>
                    
                    <div className="flex flex-col gap-2">
                        <label className={labelClass}>Currently Employed</label>
                        <div className="flex gap-4">
                            <label className="flex items-center gap-2 text-sm cursor-pointer"><input type="radio" value="yes" checked={data.is_employed === 'yes'} onChange={() => handleStatusChange('yes')} className="w-4 h-4" /> Yes</label>
                            <label className="flex items-center gap-2 text-sm cursor-pointer"><input type="radio" value="no" checked={data.is_employed === 'no'} onChange={() => handleStatusChange('no')} className="w-4 h-4" /> No</label>
                        </div>
                    </div>

                    {data.is_employed === 'yes' && (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div className="sm:col-span-2"><label className={labelClass}>Company Name</label><input type="text" required value={data.company} onChange={e => setData('company', e.target.value)} className={inputClass} /></div>
                            <div><label className={labelClass}>Employment Type</label><select required value={data.employment_type} onChange={e => setData('employment_type', e.target.value)} className={inputClass}><option value="">Select Type</option><option value="Permanent/Regular">Permanent/Regular</option><option value="Probationary">Probationary</option></select></div>
                            <div><label className={labelClass}>Position</label><input type="text" required value={data.position} onChange={e => setData('position', e.target.value)} className={inputClass} /></div>
                            <div><label className={labelClass}>Start Year</label><select required value={data.employment_start_year} onChange={e => setData('employment_start_year', e.target.value)} className={inputClass}><option value="">Select</option>{employmentYearOptions.map(y => <option key={y.value} value={y.value}>{y.label}</option>)}</select></div>
                            <div><label className={labelClass}>End Year</label><select required value={data.employment_end_year} onChange={e => setData('employment_end_year', e.target.value)} className={inputClass}><option value="current">Present</option>{employmentYearOptions.map(y => <option key={y.value} value={y.value}>{y.label}</option>)}</select></div>
                            <div><label className={labelClass}>Location</label><input type="text" required value={data.location} onChange={e => setData('location', e.target.value)} className={inputClass} /></div>
                            <div><label className={labelClass}>Monthly Salary</label><input type="number" value={data.monthly_salary} onChange={e => setData('monthly_salary', e.target.value)} className={inputClass} /></div>
                        </div>
                    )}
                    ...
                    {data.is_employed === 'no' && (
                        <div>
                            ...
                            <select required value={data.reason_unemployed} onChange={e => setData('reason_unemployed', e.target.value)} className={inputClass}>
                                <option value="" disabled>Please select your reason</option>
                                <option value="Studying">Studying</option><option value="Job Hunting">Job Hunting</option><option value="Career Break">Career Break</option>
                            </select>
                        </div>
                    )}
                </section>
